...
| **Pre-Commit: `sync-agents`** | `.github/agents/` no longer matches the roles in `.agents/pesudev-skills/auth/agents/`: usually a Dependabot submodule bump that changed a role's frontmatter, or someone edited a wrapper by hand | On your own PR: `uv run python scripts/sync_agents.py` and commit the regenerated `.github/agents/`. If it says the submodule is missing, run `git submodule update --init` first. On a Dependabot PR you cannot push to its branch: follow "Dependabot submodule bumps" in `docs/ci-cd.md` |
